...
builder.add_object({"modname": "repo_helper"})
builder.add_object({"version": "0.0.1"})
builder.add_object({"copyright_years": "2014-2019"})
builder.add_object({"copyright_years": 2020})
builder.add_object({"repo_name": "repo_helper"})  # optional
builder.add_object({"pypi_name": "repo_helper"})  # optional
builder.add_object({"import_name": "repo_helper"})  # optional
...
# Packaging
builder.add_object({"manifest_additional": ["recursive-include: repo_helper/templates *"]})  # optional
builder.add_object({"py_modules": ["domdf_spreadsheet_tools"]})  # optional
builder.add_object({"console_scripts": ["repo_helper = repo_helper.__main__:main"]})  # optional
...
pathlib.Path("repo_helper/repo_helper_schema.json").write_text(sdjson.dumps(schema, indent=2))
